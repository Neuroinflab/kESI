...
## there is at least one system in which mamba installed in the base environment is not available in other environments
#  mamba create --name $name --no-default-packages --yes -c conda-forge -c bioconda snakemake mamba python=$python_version
  conda activate $name
