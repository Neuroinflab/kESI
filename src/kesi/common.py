...
class SourceBase(_Base):
    ...
    kESI (and its special case kCSD) reconstructs CSD as a mixture of basis functions
    (in short: bases). Kernel construction tools derive all bases as translation of
    an appropriate model basis (with centroid at the origin of the coordinate system:[0, 0, 0]).

    While kernel construction tools operate on profiles of bases defined as
    callables accepting vector arguments x, y and z, we can use a
    convenience kCSD base object (of either SphericalSplineSourceKCSD or GaussianSourceKCSD3D class from kesi.common)
    which couple functions in potential and CSD space. The object is named source, as it
    represents potential (.potential() method) generated by certain CSD profile (.csd() method) in
    infinite homogeneous isotropic (kCSD assumptions) medium.
    """
