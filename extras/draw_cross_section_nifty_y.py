...
def main():
    ...
    parser.add_argument("-x", type=float, help="Slice at X coordinate (in mm)", default=0)
    parser.add_argument("-z", type=float, help="Slice at Z coordinate (in mm)", default=0)
    parser.add_argument("-g", type=float, help="position on Y axis to use as common reference, by default none", default=None)
    ...
    for file in args.files:
        ...
        x_vox, y_vox, z_vox = apply_affine(inv_affine, [args.x, 0, args.z])
        slice_of_interest = np.s_[int(x_vox), :, int(z_vox)]
